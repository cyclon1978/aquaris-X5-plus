...
{
	// minimum 0,75us, these are ns
	// 200 - GOOD LOW
	// 180 - not readable
0, // zero, not used
200, // min brightness 1 (not used)
200, // min brightness 2 (LOS) +400
600, // +500
1100, // +500
1600,  // +500
2100, // +500
...
static void mdss_dsi_panel_bklt_pwm(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	if (ctrl->pwm_period >= USEC_PER_SEC) {
		...
		pr_info("%s period_ns %d\n",__func__,period_ns);
		pr_info("%s bklt_max %d\n", __func__,ctrl->bklt_max);

		// duty_ns -> active time,  period_ns -> total time
		
		pr_info("%s level %d\n", __func__,lookupBlLevel[level]);
...
/*
		pr_info("%s active time %d of %d\n",
			level * period_ns / ctrl->bklt_max,
			period_ns);

		pr_info("%s level %d period_ns %d bklt_max %d result %d\n",
	         __func__,
	         level,
	         period_ns,
	         ctrl->bklt_max,
